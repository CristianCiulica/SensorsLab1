...
namespace sensors
{
	SensorData::SensorData(std::uint16_t id, SensorType type, std::string_view timestamp, const std::vector<float>& values, bool status)
		: m_id{ id }, m_type{ type }, m_timestamp{ timestamp }, m_values{ values }, m_status{ status }
	{
	}
	...
	const std::vector<float>& SensorData::GetValues() const
	{
		return m_values;
	}
